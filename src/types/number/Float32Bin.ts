...
export default new class Float32Bin extends Bin<number> {
    ...
    findProblem(value: any, strict = false): string | void {
        ...
        if (strict) {
            if (value > 3.4028235e38 || value < -3.4028235e38) return "Expected a number between -3.4028235e38 and 3.4028235e38";
            if (Math.abs(value) < 1.4e-45 && value !== 0) return "Expected a number not too small. (-1.4e-45, 1.4e-45) is too small";
        }
    };
}